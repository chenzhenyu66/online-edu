  ...
  <div class="main">
    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item class="input-prepend restyle" prop="memberName"
                      :rules="[{ required: true, message: '请输入你的昵称', trigger: 'blur' }]">
          <div>
            <el-input type="text" placeholder="你的昵称" v-model="params.memberName"/>
            <i class="iconfont icon-user"/>
          </div>
        </el-form-item>

        <el-form-item class="input-prepend restyle no-radius" prop="phone"
...
<!--            <a href="javascript:" :disabled="codeFun" type="button" @click="getCodeFun()" :value="codeTest"-->
<!--               style="border: none;background-color:none;">{{codeTest}}</a>-->
            <el-button type="primary" :disabled="codeFun" @click="getCodeFun()" style="width: 120px">{{codeTest}}</el-button>
          </div>
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      getCodeFun() {
        this.timeDown()
        // 这里用模拟验证码（默认111111）
        registerApi.sendCode(this.params.phone).then(response => {
          this.$message({
            type: 'success',
            message: '发送验证码成功'
          })
